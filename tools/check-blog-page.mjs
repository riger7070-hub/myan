...
for (let i = 0; i < 글수; i++) {
  ...
  // ── 5. 글자색과 글자 배경이 딸려 가는가 ──
  //
  // ⚠️ 블로그가 회색 바탕이라, 색을 안 들고 가면 편집기 기본 검정으로 떨어져
  //    읽기 어렵다. 문단마다 색을 물려 놨는데 그게 실제로 클립보드까지 가는지
  //    본다. 색을 정하는 자리(build-blog-page.mjs)에서 읽어 와 견준다 —
  //    여기 색을 다시 적으면 한쪽만 고치고 넘어간다.
  const 색소스 = readFileSync(join(ROOT, 'tools', 'build-blog-page.mjs'), 'utf8');
  const 글자색 = /const 글자색 = '([^']+)'/.exec(색소스)?.[1];
  const 글자배경 = /const 글자배경 = '([^']+)'/.exec(색소스)?.[1];
  // ⚠️ 브라우저가 클립보드에 실을 때 #ffffff 를 rgb(255, 255, 255) 로 바꿔 쓴다.
  //    16진수 그대로 찾으면 색이 멀쩡히 들어 있는데도 0개로 세어진다(실제로 그랬다).
  //    쓰는 쪽 표기가 아니라 **같은 색인지**를 견줘야 한다.
  const rgb = (hex) => {
    const h = hex.replace('#', '');
    const n = h.length === 3 ? [...h].map((x) => x + x) : h.match(/../g);
    return `rgb(${n.map((x) => parseInt(x, 16)).join(', ')})`;
  };
  const 문단수 = (html.match(/<p[ >]/gi) || []).length;
  const 색문단 = [...html.matchAll(/<p[^>]*style="[^"]*"/gi)]
    .filter((m) => m[0].includes(rgb(글자색)) && m[0].includes(rgb(글자배경))).length;
  ...
  if (!글자색 || !글자배경) 잘못.push('build-blog-page 에서 글자색을 못 읽었다');
  else if (색문단 < 문단수) {
    잘못.push(`${이름}: 문단 ${문단수}개 중 ${색문단}개만 색을 들고 갔다 — `
      + '나머지는 회색 바탕에 검은 글씨로 떨어진다');
  }
}
