...
for (const f of 글) {
  ...
  test(`${f} 에 허락하지 않은 기호가 없다`, () => {
    ...
    // ⚠️ 이모지는 **표 안에서만** 쓴다.
    //    글줄에 흩뿌리면 AI 가 쓴 티가 나는데, 표에서는 이야기가 다르다. 오행은
    //    애초에 색 이야기라 색 동그라미가 곧 내용이고, 달마다 계절을 붙여 두면
    //    이사철이 어디인지가 한눈에 보인다. 뜻을 나르면 정보이고, 안 나르면 장식이다.
    //
    //    그래서 표줄(들여쓴 줄)은 걷어내고 남은 글줄만 본다.
    const 글줄만 = src.split('\n').filter((l) => !/^\s{2,}\S/.test(l)).join('\n');
    const 본문 = 글줄만.replace(/[★※◈✅❌]/g, '');
