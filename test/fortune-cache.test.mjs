...
//
// 이 캐시가 존재하는 이유는 순전히 Gemini 의 분당 요청 한도다. 배포본 측정 결과
// 순차 12건은 전부 성공했지만 동시 5건은 5건 다 실패했다(모두 ~6.4초 뒤 422).
// 그래서 여기서 지켜야 할 계약은 "결과가 같다"가 아니라 **"호출 횟수가 줄어든다"** 이다.
...
test('동시에 들어온 5건이 Gemini 를 한 번만 부른다', async () => {
  ...
  assert.equal(calls.n, 1, `동시 5건이 Gemini 를 ${calls.n}번 불렀다 — 한도에 그대로 걸린다`);
