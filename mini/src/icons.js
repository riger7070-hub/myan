...
export const ICONS = {
  ...
  // 궁합과 인연: 맞물린 두 고리. 포개지 않고 **걸치게** 그린 이유는,
  // 궁합이 둘을 하나로 만드는 것이 아니라 서로 걸려 있는 결이기 때문이다.
  secLove:   S(`<circle cx="9" cy="12" r="5.2"/><circle cx="15" cy="12" r="5.2"/>`),
  // 이름과 인상: 낙관(落款). 이름을 새겨 찍는 도장이라 이 묶음의 머리로 맞다.
  secName:   S(`<rect x="4" y="4" width="16" height="16" rx="2.5"/>
                <path d="M8.5 8.5h7M12 8.5v7M8.5 15.5h7" opacity=".75"/>`),
  ...
  // 귀인: 손 내미는 사람과 그 손을 잡는 사람. 둘 사이에 별 하나.
  gwiin: S(`<circle cx="7" cy="6.5" r="2.6"/>
            <path d="M2.5 20.5v-1.8A4.2 4.2 0 0 1 6.7 14.5h.6"/>
            <circle cx="17" cy="6.5" r="2.6"/>
            <path d="M21.5 20.5v-1.8a4.2 4.2 0 0 0-4.2-4.2h-.6"/>
            <path d="M12 11.2 12.9 13l2 .3-1.45 1.4.34 1.98L12 15.75l-1.79.93.34-1.98L9.1 13.3l2-.3z"/>`),
  // 전생: 되감기는 실타래
  ...
  // 안스님: 눈을 감고 바리때를 든 스님. 동냥 칸에 쓴다.
  //
  // ⚠️ 눈은 **감은 선**으로 그린다. 점을 찍으면 이 크기에서 뭉개져 놀란 얼굴이 되고,
  //    스님이 놀라 있으면 받는 사람 마음이 편치 않다.
  // ⚠️ 엽전은 바리때 **위**에 띄운다. 안에 그리면 선이 겹쳐 검은 덩어리가 된다.
  monk: S(`<circle cx="12" cy="5.4" r="2.6"/>
           <path d="M6.6 15.8c0-3.8 2.4-6.6 5.4-6.6s5.4 2.8 5.4 6.6H6.6Z"/>
           <path d="M9 19.6h6a3 3 0 0 1-6 0Z"/>`),
