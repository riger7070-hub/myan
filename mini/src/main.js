 ...
 */
/**
 * 이름 뒤에 붙는 조사. 받침이 있으면 '이', 없으면 '가'.
 *
 * ⚠️ 넷 중 안도령만 받침이 있다(령). 그래서 하나로 못 박으면 "안낭자이 짚어 드린"
 *    같은 말이 나간다 — 실제로 그렇게 썼다가 시험에 걸렸다. 공유 글은 모르는
 *    사람에게 가는 글이라 이런 데서 티가 난다.
 */
function _조사(이름, 받침있을때 = '이', 없을때 = '가') {
  const c = String(이름).charCodeAt(String(이름).length - 1);
  if (c < 0xAC00 || c > 0xD7A3) return 없을때;      // 한글이 아니면 그냥 둔다
  return (c - 0xAC00) % 28 ? 받침있을때 : 없을때;
}
...
function _resultShareText(r, link) {
  ...
  return [
    ...
    // ⚠️ 이름만 적으면 처음 보는 사람에게는 그냥 모르는 이름이다. 한 줄 소개를 붙여야
    //    "누가 봐 준 것" 이 되고, 그래야 읽는 사람이 그 사람에게 자기 것도 물어보고 싶어진다.
    `${sp.name}의 ${r.item.label}\n— ${sp.intro}`,
    facts,
    hook ? rest : body,
    // ⚠️ 링크만 던지면 광고다. **이 풀이가 왜 이 사람 것인지**를 먼저 말해야
    //    받는 사람이 "내 것은 뭐라고 나올까" 를 궁금해한다. 그게 눌러 보는 이유다.
