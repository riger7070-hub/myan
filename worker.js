...
const MINI_PRODUCTS = {
  ...
  // ── 안스님 동냥 (공급가 1,000 → 판매가 1,100원) ──
  // 후원하고 싶은 사람의 자리다. 겸해서, 결제가 끝까지 도는지 적은 돈으로 볼 수 있다.
  //
  // ⚠️ **엽전 하나를 1,100원에 판다. 위 표보다 비싼 것이 맞다.**
  //    싸게 매기면 안 된다. 가장 싼 칸이 429원/개인데 동냥을 그보다 싸게 두면
  //    사람들이 엽전을 여기서 사 간다 — 값 사다리에 구멍이 뚫린다.
  //    여기서 파는 것은 엽전이 아니라 **덕담**이고, 엽전 하나는 거스름돈이다.
  //
  // ⚠️ **주는 것이 있어야 한다.** 돈만 받고 아무것도 주지 않는 상품은 심사에서 걸린다.
  //    화면에 '기부' 라고 쓰지도 않는다 — 기부는 인앱결제로 받으면 안 되는 항목이라
  //    스토어 정책에 걸릴 수 있다. 받는 것이 분명한 '동냥'으로 둔다.
  //
...
const MONK_BLESSINGS = [
  '고맙습니다. 오늘 걸음마다 그늘이 지지 않기를.',
  '잘 받았습니다. 돌아가는 길이 가볍기를.',
  '이 엽전은 제가 잘 쓰겠습니다. 오늘 끼니 거르지 마세요.',
  '고맙습니다. 급한 일은 내일로 미뤄도 괜찮습니다.',
  '잘 받았습니다. 오늘은 좀 일찍 주무세요.',
  '고맙습니다. 마음 쓰는 일에 마음을 다 쓰지는 마세요.',
  '이만하면 넉넉합니다. 애쓰셨습니다.',
];

/**
 * 주문 하나에 덕담 하나. **주문번호에서 뽑는다.**
 *
 * ⚠️ 무작위로 뽑으면 안 된다. 지급은 재시도로 여러 번 들어오는데, 그때마다 다른
