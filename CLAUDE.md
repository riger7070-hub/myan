...
| 띠·별자리 / 럭키 / 라이프패스 | includes `_ilchinYmd()` | 576 / 4 / 48 per day |

Four rules hold this together:
- **A date-scoped bucket must use `_ilchinYmd()`, never `_kstYmd()`.** Those three prompts embed `ilchin()`'s 오늘의 오행, and `ilchin()` rolls over at the Worker's local (UTC) midnight — 09:00 KST. Keying the bucket on KST midnight instead made one bucket span two different 일간: the first request between 00:00 and 09:00 KST generated the text from *yesterday's* element and every user got that for the rest of the KST day (간지 differs every day, the 오행 label about every other day). `_kstYmd()`/`_kstYear()` are for the *calendar* dates a user picks (택일's start date, 대운/궁합 시기's "this year"), which have nothing to do with `ilchin()` — **don't merge the two helpers**, one of them will always be wrong. `test/fortune-bucket-date.test.mjs` asserts `ilchin()` is constant within a bucket, and does it under a shim that forces UTC-local semantics — without that shim the test passes on a KST dev machine no matter what.
