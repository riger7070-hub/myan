...
This is a solo-owner repo (`riger7070-hub/myan`) with no staging environment. `.github/workflows/deploy-worker.yml` deploys straight to production on every push to `main` — there is no test suite and no dry-run/lint gate in CI, so a pushed change ships live within minutes.
...
There is no test suite. Verify changes manually: run a Saju reading for a fixed birth date before and after your change and confirm the four pillars (年/月/日/時) are identical, check the token balance display, and exercise the Toss payment flow with test keys.

Push to `main` triggers `.github/workflows/deploy-worker.yml` (`npm install` → `wrangler deploy`) — no separate CI build step, so whatever is in `worker.js` on `main` is what ships.
