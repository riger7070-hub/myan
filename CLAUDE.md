...
**All of GitHub's Dependabot alerts on this repo are here, and the decision is to leave them.** They are four advisories (`image-size` via `metro`, `uuid` via `xcode`, `nanoid` via `expo-router`/`postcss`); the root and `mini/` audit clean. Note the reasons differ and one of them is not "build-time only": `image-size`/`uuid` are reached only when your own assets are processed on your own build machine, but `nanoid` **does ship in the app bundle** — it is left because the sole code change in the fix (3.3.17 → 3.3.18) is a guard in `async/index.native.js`, a file nothing here imports and which cannot even load (its `expo-random` dependency isn't installed); our two call sites use `nanoid/non-secure`, which cannot loop at any size. Verified by diffing the two tarballs, not by reading the advisory text. Do **not** run `npm audit fix --force` — npm's "fix" is a downgrade to `expo@53` / `react-native@0.72` that would undo the verified Expo 57 upgrade. The reasoning is written out in `myan-native/SETUP.md` §7 so it doesn't get re-investigated each time the banner appears.
```bash
cd myan-native
npm install
npx expo start
