...
`npm run sale` 로 지금 걸린 할인을 보고 고친다. 이 시크릿도 통째로 덮어쓰므로 손으로
넣지 말 것.

⚠️ **만료일을 2030-12-31 로 멀리 밀어 두었다(2026-08-31, 사장님 요청).**
할인을 계속 유지하기 위해서인데, **무엇을 포기한 것인지 알고 있어야 한다.**

만료일은 원래 죽은 사람 스위치였다. 콘솔에서 할인을 내렸는데 `MINI_SALE` 을 안 고치면
화면은 계속 할인가를 말하고 결제창은 정가를 받는다 — **화면이 결제창보다 싸게 말한다.**
날짜가 가까이 있으면 늦어도 그날 저절로 멈췄다. 이제는 안 멈춘다.

⚠️ 그리고 이건 **자동으로 못 잡는다.** 토스는 소모품 상품에 `displayAmount`(할인 **전**
가격) 하나만 주고 실제 청구액은 알려 주지 않는다. 서버도 앱도 콘솔의 진짜 할인율을 볼
방법이 없다. 그래서 지킴이를 만들 수가 없다 — **콘솔에서 할인을 손대면 그 자리에서
`npm run sale` 을 함께 돌리는 것 말고는 길이 없다.**

할인을 영영 유지할 생각이면, 콘솔 **정가 자체를 그 값으로 내리고** `MINI_SALE` 을 비우는
편이 안전하다. 어긋날 두 값이 애초에 없어진다. 다만 「30% 할인」 딱지는 사라진다.
